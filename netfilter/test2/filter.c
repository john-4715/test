					  ...
					  const struct net_device *out, int (*okfn)(struct sk_buff *))
{
	...
	if (iph->daddr == in_aton("192.168.18.128"))
	{
		...
		if (iph->protocol == IPPROTO_TCP)
		{
			...
			{
				iph->saddr = in_aton("192.168.18.128");
				dev = dev_get_by_name(&init_net, "ens33");
				tcph->check = 0;
				skb->csum = csum_partial((unsigned char *)tcph, tot_len - iph_len, 0);
				tcph->check =
					csum_tcpudp_magic(iph->saddr, iph->daddr, ntohs(iph->tot_len) - iph_len, iph->protocol, skb->csum);
				iph->check = 0;
...
static int __init filter_init(void)
{
	...
	ret = nf_register_net_hook(&init_net, &nfho, sizeof(nfho));
	if (ret < 0)
	{
...
static void filter_fini(void) { nf_unregister_net_hook(&init_net, &nfho); }
